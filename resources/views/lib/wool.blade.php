...
@section('content')
    <div class="page-lib">
        <div class="ui center aligned blue header">Справочник типов шерсти</div>
        <div class="ui bottom attached active tab" data-tab="list">


            <div class="ui field">
                <label for="">Вид животного</label>
                <div class="ui selection dropdown sel_kind">
        ...
        <div class="ui bottom attached tab" data-tab="edit">
            ...
            <form class="ui form frm-add" onsubmit="return false;" method="post">
                ...
                <div class="ui inline field">
                    <label for="">Вид животного</label>
                    <div class="ui selection dropdown sel_edit_kind">
                ...
                <div class="ui inline required field">
                    ...
                    <input style="width:calc(100% - 100px)" id="name" name="wooltype_name" type="text" class="inp_change">
                </div>
    ...
    <script>
        ...
        let page = {
            ...
            init() {
                ...
                window.app.dropdownLoad('.sel_kind', {
                    api: {
                        url: '/lib/kind/get',
                        data: {
                            type: 'dropdown'
                    ...
                    value: 1,
                    thenLoad: function () {
                        $('.sel_kind').dropdown('setting', 'onChange', function () {
                            self.table.ajax.reload();
                        });
                    }
                });
                window.app.dropdownLoad('.sel_edit_kind', {
                    api: {
                        url: '/lib/kind/get',
                        data: {
                            type: 'dropdown'
                ...
                self.table = self.body.find('.tbl_lib').DataTable({
                    ...
                    ajax: {
                        url: "/lib/wooltype/get",
                        type: 'POST',
                        data: function (d) {
                            d.kind = self.body.find('.sel_kind').dropdown('get value');
                            d.type = 'table';
                        },
                        dataSrc: 'data'
                    },
            ...
            loadEdit(rec) {
                ...
                self.body.find('.lbl_page_name').html("Изменение")
                $.tab('change tab', 'edit');
            },
            loadAdd() {
                let self = this;
